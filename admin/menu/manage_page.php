    ...
    if($page>-1){
                        ...
                        echo "<table class='type03'><tr><th id='small'>번  호</th><th>아티스트</th><th>곡  명</th><th>주소값</th><th id='small'>장 르</th><th>등록일자</th></tr>";
                          while($row=mysqli_fetch_row($result)){
                          echo "<form method='POST' action = './delete_action.php'>";
                          echo "<input type='hidden' name='key' value='$row[0]'/>";  
                          echo "<tr><td id='small'>$row[0] </td><td> $row[1] </td><td> $row[2] </td><td> $row[3] </td><td id='small'> $row[4] </td><td> $row[5] </td>";
